...
uint8_t BSP_LCD_Init(void)
{
  ...
  /* 이 함수가 SDRAM을 "진짜로" 깨우는 곳이다. CubeMX가 생성한 MX_FMC_Init()은
     SDRAM 컨트롤러 타이밍 설정만 하고, 정작 SDRAM 칩에게 "이제 시작해라"고
     알려주는 JEDEC 초기화 명령(프리차지 → 오토리프레시 → 모드레지스터 설정
     → 리프레시 주기 등록)은 빼먹었다 — CubeMX가 자동생성 못 하는 부분이라
     원래 사용자가 직접 채워야 하는데 비어 있었음. 그 명령 시퀀스를 여기
     BSP_SDRAM_Init()이 전부 포함하고 있으므로 반드시 호출해야 한다. */
  BSP_SDRAM_Init();
