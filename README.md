...
- **수정안(단계 분리)** :
  ...
  | 2 | `PLLM` 16→4, `PLLN` 72→168, `PLLSAIDivR` 4→**16** (픽셀 클럭을 현재와 동일한 1.5MHz로 유지), APB1 /2→/4, APB2 /1→/2, 전압 스케일 3→1, 플래시 레이턴시 0→5, `REFRESH_COUNT`→1292 | 중간 — 화면 경로는 무변경이나 CubeMX Generate 회귀 위험([13-2](#13-2-cubemx-generate가-사용자-자산과-빌드-설정을-삭제)) | 예정 |
