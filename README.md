...
| 터치 I2C3 (SCL/SDA) | PA8 / PC9 | STMPE811 — **BSP 드라이버가 단독 소유** ([13-2](#13-2-i2c3-이중-초기화로-인한-부팅-행) 참고) |
...
"감지됐다"가 아니라 **"충분히 오래 안 잡혔다"** 를 복귀 조건으로 삼는 게 핵심입니다 ([13-3](#13-3-rc522-카드-인식-깜빡임으로-인한-화면-요동) 참고).
...
### 13-3. RC522 카드 인식 깜빡임으로 인한 화면 요동
...
### 13-4. 클럭 트리 — PLLM 공유로 인한 SYSCLK 저하와 SDRAM 리프레시 불일치

> **현재 상태 : 원인 분석 완료. 1단계(SDRAM 리프레시 교정) 반영 완료, 2·3단계(클럭 트리 정상화)는 예정.**
> 잠복 위험이 컸던 리프레시 불일치부터 먼저 잡았습니다. SYSCLK 저하 자체는 현재 동작에 문제가 없어 뒤로 미뤘습니다.
...
- **원인 1 — PLLM은 공유 자원** : RM0090에 따르면 **PLLM 분주기는 메인 PLL·PLLI2S·PLLSAI가 공통으로 쓰는 앞단 분주기**입니다(`RCC_PLLSAICFGR`에는 N/Q/R만 있고 M 필드가 없어, 메인 PLL의 것을 그대로 가져다 씁니다). 즉 **화면 클럭을 낮추려고 PLLM을 건드리면 CPU 클럭까지 함께 내려갑니다.**
...
- **원인 2 — SDRAM 리프레시 값이 상류 변경을 따라가지 못함** : SDRAM은 데이터가 저절로 새기 때문에 주기적으로 다시 채워야 하고(리프레시), 이 주기는 FMC에 **클럭 사이클 개수**로 지정합니다.
  ...
  | **18MHz** | **9MHz** | **120** | **현재 — 542가 남아 있던 것을 이 값으로 교정함** |
...
- **수정안(단계 분리)** :
  ...
  | 단계 | 내용 | 위험도 | 상태 |
  |---|---|---|---|
  | 1 | `REFRESH_COUNT` 542 → **120** (현재 SDCLK 9MHz 기준) | 낮음 — 헤더 상수 1줄, 리프레시를 더 자주 하는 방향(FMC 최소 41 충족) | **반영 완료** |
  | 2 | `PLLM` 16→4, `PLLN` 72→168, `PLLSAIDivR` 4→**16** (픽셀 클럭을 현재와 동일한 1.5MHz로 유지), APB1 /2→/4, APB2 /1→/2, 전압 스케일 3→1, 플래시 레이턴시 0→5, `REFRESH_COUNT`→1292 | 중간 — 화면 경로는 무변경이나 CubeMX Generate 회귀 위험 | 예정 |
  | 3 | `PLLSAIDivR` 16→8→4로 한 단계씩 시험 (4가 원래 의도한 6MHz) | 화면 재검증 필요 | 예정 |
...
동일한 폰트·크기라면 픽셀 단위로 동일하게 재현되는 결정론적 렌더링입니다. 새 글리프를 추가한 직후에는 **CubeMX를 다시 만지기 전에 반드시 커밋**하세요.

> **CubeMX에서 Generate Code 실행 후에는 반드시 `git status`를 확인하세요.**
