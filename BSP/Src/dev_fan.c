 ...
 *  히스테리시스(켜는 기준과 끄는 기준을 다르게 두는 기법)를 적용한다. 래치가 걸려 있는
...
/* 직전 tick에 실제로 CCR에 써 넣은 duty(%) — 이번 값과 비교하는 기준.
   초기값 0xFFFF는 "아직 한 번도 반영한 적 없음"을 뜻하는 무효값이라,
   부팅 후 첫 계산 결과가 0%여도 반드시 한 번은 CCR에 기록된다. */
...
void Fan_Update(void)
{
    ...
    /* ── 1) 비상 강제 차단(최우선 판정) ──
       물리 비상정지(오류코드 401)가 올라오면 다른 조건을 전부 무시하고 팬을 즉시 끈다.
       화재 발생 시 팬 바람이 불길을 키우는 것을 막기 위한 안전 조치이므로,
       고온 경보 래치가 걸려 있어도 무조건 0%로 강제하고 곧바로 빠져나간다. */
    ...
    /* ── 2) 원시 온도(℃) 읽기 ──
       표시용 오류코드가 아니라 카에서 받은 실제 온도값을 그대로 사용한다. */
    ...
    /* ── 4) 목표 duty(%) 결정 ──
       래치가 걸려 있으면 무조건 100%, 아니면 온도 구간표를 따른다. */
    ...
    {
        /* 래치가 풀린 상태: 아래 온도 구간표대로 duty를 정한다.
           26.0℃ 미만 -> 0%, 26.0~27.9℃ -> 30%, 28.0~29.9℃ -> 60%, 30.0℃ 이상 -> 100% */
        if (temp < 26.0f)
            target_duty = 0;
        else if (temp < 28.0f)
            target_duty = 30;
    ...
    /* ── 7) 변경분만 CCR에 반영 ──
       duty가 직전 tick과 같으면 레지스터에 다시 써 봐야 결과가 똑같으므로,
       매 tick 반복되는 불필요한 레지스터 쓰기를 건너뛴다. */
