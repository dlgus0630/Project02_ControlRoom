/* 한글 글리프를 LCD에 그려 주는 헬퍼 구현.
   글리프는 1비트 팩킹(픽셀 하나를 비트 하나로 저장) 비트맵이다:
     - 행 stride(한 행이 차지하는 바이트 수) = (width + 7) / 8 바이트
     - 각 바이트의 MSB(0x80)가 왼쪽 픽셀
     - 비트=1이 글자 픽셀, 비트=0이 배경 픽셀 */
                   ...
                   uint32_t fgColor, uint32_t bgColor)
{
    ...
    /* 한 행이 차지하는 바이트 수(폭이 8의 배수가 아니면 바이트 경계까지 패딩됨). */
    ...
    for (row = 0; row < glyph->height; row++)
    {
        for (col = 0; col < glyph->width; col++)
        {
            ...
            bit_mask   = (uint8_t)(0x80 >> (col % 8)); /* MSB가 왼쪽 픽셀 */
